...
void bfs(struct Graph* graph, int src, int dest, BFSResult* result) {
...
int edmondsKarp(struct Graph* graph, int src, int dest) {
    int min_capacity;
    int max_flow = 0;

    while (true) {
        BFSResult bfs_result;
        bfs(graph, src, dest, &bfs_result);
...
int main() {
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
        int max_flow = edmondsKarp(graph, 0, graph->num_nodes-1);
        printf("\nMax flow: %d\n", max_flow);
